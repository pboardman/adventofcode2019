...
class IntcodeComputer:
    def __init__(self, program_code, pause_on_output=False, ascii_mode=False, logger=logging):
        # Set logging level
    ...
    def output(self):
        ...
        if self.ascii_mode:
            self.outputs.append(chr(value))
            print(chr(value), end='')
            if chr(value) == '?' and ''.join(self.outputs[-8:]) == 'Command?':
                # self.outputs = []
                self.pause = True
        else:
